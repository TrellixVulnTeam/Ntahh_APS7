...
const ReadFitur = () => {
	...
	console.log(chalk.green('Loading system . . .'))
	...
	console.log(chalk.yellow('Notice\nWaiting for respone system . . .'))
	console.log(chalk.blue('Nungguin Ya 🗿'))
};
// cmd
ReadFitur();
...
async function start(){
  ...
  console.clear();
  console.log(color('Hello, User!\nWelcome To Console Logs', 'yellow'))
  // Jika eror figlet (Doom) , hapus console yg ada di bawah ini!! [ Teruntuk user heroku :v ]
    
  const conn = Baileys({
    printQRInTerminal: true,
		auth: state,
		browser: ["One BotZ-MD", "Safari", "3.0.0"],
  ...
  const decodeJid = (jid) => {
	...
	conn.ev.on("connection.update", async (ktl) => {
	  ...
	  if (connection == "connecting") console.log(chalk.cyan('Menyambungkan Ke WhatsApp Anda . . .'))
	  if (connection) {
