                ...
                if (chat.welcome) {
                    ...
                    for (let user of participants) {
                        ...
                        try {
                            ...
                            text = (action === 'add' ? (chat.sWelcome || this.welcome || conn.welcome || 'Hai @user, Met Datang Ke Grup @subject\n@desc').replace('@subject', await this.getName(id)).replace('@desc', groupMetadata.desc ? String.fromCharCode(8206).repeat(4001) + groupMetadata.desc : '') :
                                (chat.sBye || this.bye || conn.bye || '@user Keluar Karena Ingin Keluar')).replace('@user', await this.getName(user))
                            let wel = API('males', '/welcome2', {
                                profile: pp,
                                username: await this.getName(user),
                                background: 'https://uploader-one-botz.herokuapp.com/file/ArfmFFmmUeAi.jpeg',
                            ...
                            await this.send3TemplateButtonImg(id, action === 'add' ? wel : lea, text, wm, action === 'add' ? 'Met Datang..' : 'See U Again Beban 🐤', action === 'add' ? 'huuu' : 'huuu')
                        }
                    }
                }
                break
...
global.dfail = async (type, m, conn) => {
    let msg = {
        rowner: `*Hanya Owner Yang Bisa Mengakses Perintah Ini*`,
        owner: `*Hanya Owner Yang Bisa Mengakses Perintah Ini*`,
        mods: `*Hanya Moderator Yang Bisa Mengakses Perintah Ini*`,
        premium: '*Hanya User Premium Yang Bisa Mengakses Perintah Ini*',
        group: `Gunakan Inj Di *Grup*`,
        private: 'Gunakan Ini Di *Private Chat*',
        admin: 'Anda *Bukan Admin :\*',
        botAdmin: 'Jadikan bot sebagai *Admin*',
        unreg: 'Silahkan daftar untuk menggunakan fitur ini dengan cara mengetik:\n\n*#daftar nama.umur*\n\nContoh: *.daftar ${conn.getName(m.sender)}.16*',
